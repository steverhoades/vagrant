...
if hash_key_equals($mongodb_values, 'install', 1) {
  ...
  $mongodb_service_name = $::operatingsystem ? {
    'ubuntu' => 'mongod',
    undef     => $mongodb::params::service_name,
    ''        => $mongodb::params::service_name,
    default   => $mongodb::params::service_name,
  }

  $mongodb_settings = merge($mongodb_values['settings'], {
    'service_name' => $mongodb_service_name,
  })

  create_resources('class', { 'mongodb::server' => $mongodb_settings })
